...
<table width="100%" align='center' style='font-size: 13px;'
	...
	<tr>
		<td
			...
			总${pageModel.pages}页数&nbsp;&nbsp;总记录条数${pageModel.total}
			<c:choose>
				<c:when test="${pageModel.pageNum==1}">
					...
					<c:choose>
						<c:when test="${pageModel.pageNum==pageModel.total}">
							<span class='disabled'>下一页</span>
						</c:when>
						<c:otherwise>
							<span class='current'> <a href='#'
								onclick="toPage(${pageModel.pageNum+1})">下一页</a></span>
						</c:otherwise>
					</c:choose>
				</c:when>
				<c:when test="${pageModel.pageNum==pageModel.total}">
					<span class='current'> <a href='#'
						onclick="toPage(${pageModel.pageNum-1})">上一页</a>
					</span>
					<c:forEach begin="1" end="${pageModel.pages}" var="i">
						<c:choose>
							<c:when test="${pageModel.pageNum==i}">
				...
				<c:otherwise>
					<span class='current'> <a href='#' onclick="toPage(${pageModel.pageNum-1})">上一页</a>
					</span>
					<c:forEach begin="1" end="${pageModel.pages}" var="i">
						<c:choose>
							<c:when test="${pageModel.pageNum==i}">
